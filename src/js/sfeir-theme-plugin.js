...
export class SfeirTheme {
        ...
        for (let subSection of subSections) {
          ...
          if (subSection.hasAttribute("data-background")) {
            parentSection.classList.add(
              indexSection === 0 ? "data-bg-left" : "data-bg-right"
            );
            const dataBgString = subSection.getAttribute("data-background");
            if (
              /^(http|file|\/\/)/gi.test(dataBgString) ||
              /\.(svg|png|jpg|jpeg|gif|bmp|webp)([?#\s]|$)/gi.test(dataBgString)
            ) {
  ...
  _manageSpeakersBorders() {
    const imgOfSpeakersToReplaces = [
      ...document.querySelectorAll(".reveal .slides section img[alt*=speaker]"),
    ];
